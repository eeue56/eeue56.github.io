...
Elm.People = Elm.People || {};
Elm.People.make = function (_elm) {
   "use strict";
   _elm.People = _elm.People || {};
   if (_elm.People.values)
   return _elm.People.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   var updatePeople = F2(function (newName,
   names) {
      return A2($Basics._op["++"],
      names,
      _L.fromArray([newName]));
   });
   var newPerson = Elm.Native.Port.make(_elm).inboundSignal("newPerson",
   "String",
   function (v) {
      return typeof v === "string" || typeof v === "object" && v instanceof String ? v : _U.badPort("a string",
      v);
   });
   var currentPeople = newPerson;
   var names = A3($Signal.foldp,
   updatePeople,
   _L.fromArray([]),
   currentPeople);
   var people = Elm.Native.Port.make(_elm).outboundSignal("people",
   function (v) {
